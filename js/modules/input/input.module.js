...
export const InputModule = {
  ...
  // DOM references stored as instance properties for cross-method access
  spLabel: null,
  spPause: null,
  zmLabel: null,
  ...
  init(canvas, uiEl, cursorEl, slider, pcountEl, gravSlider, gravVal) {
    ...
    // Initialize speed bar UI to match current state on load
    this._updateSpeedUI();
  },
  ...
  _bindUI() {
    ...
    // Speed bar elements — stored on instance for access from _bindKeyboard
    ...
    if (this.spPause) {
      ...
      this.spPause.addEventListener("pointerdown", e => {
        e.preventDefault();
        togglePause();
        this._updateSpeedUI();
      });
    ...
    document.getElementById("zm-in")?.addEventListener("pointerdown", e => {
      ...
      if (this.zmLabel) this.zmLabel.textContent = (CameraModule.cam.targetZoom * 100).toFixed(0) + "%";
    });
  ...
  /**
   * Updates the speed bar UI to match current physics state.
   * Reads from instance properties (this.spLabel, this.spPause)
   * so it can be called from any method without argument passing.
   */
  _updateSpeedUI() {
    if (this.spLabel) {
      this.spLabel.textContent = physSpeed === 0 ? "0×" : physSpeed === 1 ? "1×" : physSpeed.toFixed(1) + "×";
    }
    if (this.spPause) {
      this.spPause.textContent = paused ? "▶" : "▐▐";
      this.spPause.className = paused ? "paused" : "";
    }
  ...
  _bindKeyboard() {
    window.addEventListener("keydown", e => {
      ...
      if (e.key === " " || e.key === "p") { e.preventDefault(); togglePause(); this._updateSpeedUI(); }
      if (e.key === "]") { setPhysSpeed(parseFloat((physSpeed + 0.5).toFixed(1))); this._updateSpeedUI(); }
      if (e.key === "[") { setPhysSpeed(parseFloat((physSpeed - 0.5).toFixed(1))); this._updateSpeedUI(); }
    });
  },
