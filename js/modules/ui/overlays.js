...
export const OverlaysModule = {
    ...
    updateFPS: (rawDt) => {
        ...
        if (OverlaysModule.fpsSamples.length > 0) {
            OverlaysModule.fpsDisplay = Math.round(OverlaysModule.fpsSamples.reduce((a,b)=>a+b) / OverlaysModule.fpsSamples.length);
        }
    },
    ...
    spawnPlanet: (x, y, size, pcountEl) => {
        ...
        body.id = `body_${Date.now()}_${Math.random().toString(36).slice(2)}`;        state.bodies.push(body);
        
        state.flashes.push({ x, y, r: radius * 0.1, maxR: radius * 2, gc: pal.gc, life: 0.45, speed: 0.12, kind: "fill" });
    ...
    drawCharge: (ctx, holding, holdT, tx, ty, sliderVal) => {
        ...
        ctx.beginPath(); ctx.arc(tx, ty, 3, 0, PI2);
    ...
    predictOrbit: (spawnX, spawnY, vx0, vy0, nP, steps, dtPerStep, recordEvery, grav = sunGravMult) => {
        const pts = [];
        ...
        for (let i = 0; i < steps; i++) {
            ...
            const sd2 = sdx * sdx + sdy * sdy;            if (sd2 < burnR2) break;
    ...
    getPreviewPath: (wx, wy) => {
        ...
        const totalSteps = Math.min(Math.ceil(periodSub * 7), 120000);
        const { vx, vy } = OverlaysModule.getSpawnVelocity(wx, wy, 100, sunGravMult);
        const pts = OverlaysModule.predictOrbit(wx, wy, vx, vy, 100, totalSteps, dtPerStep, 20, sunGravMult);
    ...
    drawOrbitPreview: (ctx, holding, holdT, tx, ty) => {
        ...
        for (let i = 0; i < total - SEG; i += SEG) {
            const f0 = i / total, f1 = (i + SEG) / total, fc = (f0 + f1) / 2;            const r = Math.floor(lerp(160, 255, Math.min(fc * 1.8, 1)));
        ...
        const animFrac = (performance.now() * 0.00035) % 1, DOT_COUNT = 7;
        ...
        ctx.save(); ctx.globalAlpha = alpha * 0.28; ctx.setLineDash([3 / CameraModule.cam.zoom, 4 / CameraModule.cam.zoom]);
        ctx.beginPath(); ctx.moveTo(SUN.x, SUN.y); ctx.lineTo(w.x, w.y);
        ctx.strokeStyle = "rgba(255,200,80,1)"; ctx.lineWidth = 0.6 / CameraModule.cam.zoom; ctx.stroke(); ctx.setLineDash([]); ctx.restore();
        ...
        // Spawn point dot
        ctx.save(); ctx.globalAlpha = alpha; ctx.beginPath(); ctx.arc(w.x, w.y, 3.5 / CameraModule.cam.zoom, 0, PI2);
        ctx.fillStyle = "rgba(160,225,255,1)"; ctx.fill(); ctx.restore();

        // Burn zone circle
        ctx.save();
        ctx.beginPath(); ctx.arc(SUN.x, SUN.y, BURN_ZONE_R, 0, PI2);        ctx.strokeStyle = `rgba(255,60,30,${alpha * 0.4})`;
        ctx.lineWidth = 1.5 / CameraModule.cam.zoom;
        ctx.setLineDash([8 / CameraModule.cam.zoom, 5 / CameraModule.cam.zoom]); ctx.stroke(); ctx.setLineDash([]);
        ...
        const midSX = ((SUN.x - CameraModule.cam.x) * CameraModule.cam.zoom + CameraModule.width / 2 + (w.x - CameraModule.cam.x) * CameraModule.cam.zoom + CameraModule.width / 2) / 2;
        const midSY = ((SUN.y - CameraModule.cam.y) * CameraModule.cam.zoom + CameraModule.height / 2 + (w.y - CameraModule.cam.y) * CameraModule.cam.zoom + CameraModule.height / 2) / 2;
