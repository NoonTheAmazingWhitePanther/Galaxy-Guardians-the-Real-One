...
export const EffectsModule = {
  ...
  init(width, height) {
    ...
    // FIX: sprite must be large enough for the gradient
    ...
    grad.addColorStop(0.3, 'rgba(200, 220, 255, 0.7)'); // Soft blueish glow
  ...
  drawStars(ctx, t, width, height) {
    ...
    for (const s of this.stars) {
      ...
      // FIX: center the sprite on the star's coordinates
  ...
  drawNebula: (ctx, t, width, height) => {
    const blobs = [
      ...
      [width * 0.5, height * 0.15, Math.max(width, height) * 0.5, 4, 12, 40, 0.03],
