...
export const ControlType = {
  BUTTON:       'button',
  WIDE_BUTTON:  'wideButton',   // profile-style: spans full width, one selected
  RESET_BUTTON: 'resetButton',  // single red full-width button
  SLIDER:       'slider',
  KNOB:         'knob',
  DROPDOWN:     'dropdown',
  CHECKBOX:     'checkbox',
...
export const ButtonControl = {
  render(ctx, control, x, y, w, h, state) {
    ...
    const isAtDefault = control.governor && !control.governor.isManual;

    for (let i = 0; i < 3; i++) {
      const bx        = x + i * (BTN_W + BTN_GAP);
      const isActive  = state.pressIdx === i;
      const isHover   = state.hoverIdx === i;
      const isDefault = isAtDefault && i === 1;  // middle = highlighted when AUTO
      ...
      ctx.strokeStyle = isActive || isDefault
...
// ── Wide Button Control (profile selector) ───────────────────────────────
// Spans the full panel width. Buttons are evenly spaced.
// One is selected/highlighted at a time. Tap-only, no drag.
const WIDE_BTN_H   = 24;
const WIDE_BTN_GAP = 5;

export const WideButtonControl = {
  render(ctx, control, x, y, w, h, state) {
    const s       = DEBUG_STATE.style;
    const buttons = control.config?.buttons || [];
    const count   = buttons.length;
    if (count === 0) return;

    const btnW = (w - (count - 1) * WIDE_BTN_GAP) / count;

    for (let i = 0; i < count; i++) {
      const bx       = x + i * (btnW + WIDE_BTN_GAP);
      const btn      = buttons[i];
      const isActive = state.pressIdx === i;
      const isHover  = state.hoverIdx === i;
      ...
      ctx.beginPath();
      ctx.roundRect(bx, y, btnW, WIDE_BTN_H, 5);
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.roundRect(bx + 0.5, y + 0.5, btnW - 1, WIDE_BTN_H - 1, 5);
      ctx.stroke();
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(btn.label || String(i), bx + btnW / 2, y + WIDE_BTN_H / 2);
    }
  },
  ...
  hitTest(control, x, y, bounds) {
    ...
    const btnW = (bw - (count - 1) * WIDE_BTN_GAP) / count;

    for (let i = 0; i < count; i++) {
      const btnX = bx + i * (btnW + WIDE_BTN_GAP);
      if (x >= btnX && x <= btnX + btnW && y >= by && y <= by + WIDE_BTN_H) {
        return { btnIdx: i };
      }
    }
    return null;
  ...
  getBounds(control, x, y, w) {
    return { x, y, w, h: WIDE_BTN_H };
  }
};

// ── Reset Button Control — single full-width red button ──────────────────
export const ResetButtonControl = {
  render(ctx, control, x, y, w, h, state) {
    const s        = DEBUG_STATE.style;
    const isActive = state.pressIdx === 0;
    const isHover  = state.hoverIdx === 0;
    const baseColor = control.config?.color || 'rgba(255,70,70,0.85)';

    ctx.fillStyle = isActive
      ? baseColor.replace(/[\d.]+\)$/, '0.5)')
      : isHover
        ? baseColor.replace(/[\d.]+\)$/, '0.3)')
        : baseColor.replace(/[\d.]+\)$/, '0.15)');
    ctx.beginPath();
    ctx.roundRect(x, y, w, WIDE_BTN_H, 5);
    ctx.fill();

    ctx.strokeStyle = baseColor;
    ctx.lineWidth   = isActive ? 1.5 : 1;
    ctx.beginPath();
    ctx.roundRect(x + 0.5, y + 0.5, w - 1, WIDE_BTN_H - 1, 5);
    ctx.stroke();
    ...
    ctx.textAlign    = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(control.config?.text || 'Reset', x + w / 2, y + WIDE_BTN_H / 2);
  },

  hitTest(control, x, y, bounds) {
    const { x: bx, y: by, w: bw } = bounds;
    if (x >= bx && x <= bx + bw && y >= by && y <= by + WIDE_BTN_H) {
      return { btnIdx: 0 };
    }
    return null;
  },
...
export const ControlRenderer = {
  render(ctx, control, x, y, w, h, state) {
    switch (control.type) {
      ...
      case ControlType.WIDE_BUTTON:  WideButtonControl.render(ctx, control, x, y, w, h, state); break;
      case ControlType.RESET_BUTTON: ResetButtonControl.render(ctx, control, x, y, w, h, state); break;
  ...
  hitTest(control, x, y, bounds) {
    switch (control.type) {
      ...
      case ControlType.WIDE_BUTTON:  return WideButtonControl.hitTest(control, x, y, bounds);
      case ControlType.RESET_BUTTON: return ResetButtonControl.hitTest(control, x, y, bounds);
  ...
  getBounds(control, x, y, w) {
    switch (control.type) {
      ...
      case ControlType.WIDE_BUTTON:  return WideButtonControl.getBounds(control, x, y, w);
      case ControlType.RESET_BUTTON: return ResetButtonControl.getBounds(control, x, y, w);
