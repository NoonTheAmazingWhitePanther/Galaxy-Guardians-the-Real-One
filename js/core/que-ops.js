...
const SUBJECTS = {
  ...
  // THE BIGGER LIST — every department of the app owns a lane, so real-time
  // wiring can queue into (or skip past) any of them. Unused lanes cost zero.
...
// AFTER it this frame (FutureCache.pump today; more lanes later) can call
...
function _tick() {
  ...
  // downstream (FutureCache.pump today) measures "remaining" against.
...
export const QueOps = {  init: _init, add: _add, tick: _tick,
  ...
  get budget()        { return ManualOverrides.get('queOpsBudget', COOLING_STATES[_state.stats.coolingStateIndex]); },
  get opsProcessed()  { return _state.stats.opsProcessed; },
  get opsDeferred()   { return _state.stats.opsDeferred; },
  get opsSkipped()    { return _state.stats.opsSkipped; },
  get opsStaggered()  { return _state.stats.opsStaggered; },
  get budgetUsed()    { return _state.stats.budgetUsed; },
