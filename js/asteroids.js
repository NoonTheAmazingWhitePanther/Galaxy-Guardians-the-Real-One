...
//const { H.hypot, H.rnd, H.rndR, H.PI2 } = window.Sim;
...
window.Sim.spawnAsteroid = () => {
  ...
  const speed = H.rndR(3.5, 7.0);
  const cvx = Math.cos(aimA) * speed, cvy = Math.sin(aimA) * speed;
  const pal = window.Sim.AST_PALETTE[Math.floor(H.rnd() * window.Sim.AST_PALETTE.length)];
  const nRocks = Math.floor(H.rndR(3, 9)), clusterR = H.rndR(8, 28);
  const rocks = Array.from({ length: nRocks }, () => {
    const ra = H.rnd() * H.PI2, rd = H.rnd() * clusterR, rr = H.rndR(2, 6);
    return { ox: Math.cos(ra) * rd, oy: Math.sin(ra) * rd, dvx: (H.rnd() - 0.5) * 0.015, dvy: (H.rnd() - 0.5) * 0.015, r: rr, pts: window.Sim.makeRockShape(rr), angle: H.rnd() * H.PI2 };
  });
  ...
  for (let ai = window.Sim.state.asteroids.length - 1; ai >= 0; ai--) {
    ...
    for (const b of window.Sim.state.bodies) {
      ...
      if (d < b.radius + a.clusterR * 0.65) {
        const spd = H.hypot(a.vx, a.vy), nx = (b.cx - a.x) / d, ny = (b.cy - a.y) / d;
        for (const p of b.particles) { if (!p.dead && H.hypot(p.x - a.x, p.y - a.y) < b.radius * 0.7) { p.heat = Math.min(1, p.heat + spd * 0.05); p.vx += nx * spd * a.mass / b.mass * 0.1; p.vy += ny * spd * a.mass / b.mass * 0.1; } }
        for (const rock of a.rocks) { const ra = H.rnd() * H.PI2, rs = H.rndR(0.3, spd * 0.4); window.Sim.state.loose.push({ x: a.x + rock.ox, y: a.y + rock.oy, vx: a.vx + Math.cos(ra) * rs, vy: a.vy + Math.sin(ra) * rs, mass: rock.r * 0.3, pal: { gc: "180,160,140" }, heat: 0.5, life: H.rndR(0.6, 1), decay: H.rndR(0.003, 0.008) }); }
    ...
    if (H.hypot(a.x - window.Sim.SUN.x, a.y - window.Sim.SUN.y) > 32000) window.Sim.state.asteroids.splice(ai, 1);
  }
};
...
window.Sim.drawAsteroids = () => {
  ...
  for (const a of window.Sim.state.asteroids) {
    const sunDx = window.Sim.SUN.x - a.x, sunDy = window.Sim.SUN.y - a.y, sunD = H.hypot(sunDx, sunDy) || 1;
    ...
    ctx.fillStyle = coma; ctx.beginPath(); ctx.arc(a.x, a.y, comaR, 0, H.PI2); ctx.fill();
    for (const rock of a.rocks) {
      ...
      ctx.beginPath(); ctx.arc(-rock.r * 0.22, -rock.r * 0.22, Math.max(0.3 / window.Sim.cam.zoom, rock.r * 0.18), 0, H.PI2);
