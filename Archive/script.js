...
const Config = {
  ...
  // Safe setter to trigger recalculation if needed
  setParticleRadius(val) {
    this.PARTICLE_R = Math.max(1.5, Math.min(6, val));
    // Derived values auto-update via getters, but we can notify the engine if needed
  }
};
...
// PHYSICS & SUN// ============================================================================
const SUN = {
  x: 0, y: 0, radius: 180, burnRadius: 220,
  burnRadiusSq: 220 * 220, mass: 182784, coronaTime: 0
};
...
function spawnAsteroid() {
  if (asteroids.length >= AST_MAX) return;  const spawnR = 10000 + rndR(4000, 10000);
  const fromA = rnd() * PI2;
  ...
  const rocks = Array.from({ length: nRocks }, () => {
    ...
    const rr = rndR(2, 6);
    return { ox: Math.cos(ra) * rd, oy: Math.sin(ra) * rd, dvx: (rnd() - 0.5) * 0.015, dvy: (rnd() - 0.5) * 0.015, r: rr, pts: makeRockShape(rr), angle: rnd() * PI2 };
  });

  asteroids.push({
    x: sx, y: sy, vx: cvx, vy: cvy, rocks, clusterR,
    mass: rocks.reduce((s, r) => s + r.r * r.r, 0) * 0.2,
    pal, trail: [], tailLen: 180, age: 0
  });
}

function tickAsteroids(dt) {
  astTimer++;
  const spawnEvery = Math.max(120, AST_SPAWN_INTERVAL / Math.max(physSpeed, 0.1));
  ...
  for (let ai = asteroids.length - 1; ai >= 0; ai--) {
    ...
    for (const b of bodies) {
      ...
      if (d < b.radius + a.clusterR * 0.65) {
        ...
        for (const p of b.particles) {
          if (p.dead) continue;
          if (hypot(p.x - a.x, p.y - a.y) < b.radius * 0.7) {
        ...
        for (const rock of a.rocks) {
          ...
          loose.push({ x: a.x + rock.ox, y: a.y + rock.oy, vx: a.vx + Math.cos(ra) * rs, vy: a.vy + Math.sin(ra) * rs, mass: rock.r * 0.3, pal: { gc: "180,160,140" }, heat: 0.5, life: rndR(0.6, 1), decay: rndR(0.003, 0.008) });
        }
    ...
    if (hypot(a.x - SUN.x, a.y - SUN.y) > 32000) asteroids.splice(ai, 1);
  }
}

// ============================================================================
// DRAWING: ASTEROIDS & STARS
// ============================================================================
function drawAsteroids() {
  for (const a of asteroids) {
    const sunDx = SUN.x - a.x, sunDy = SUN.y - a.y;
    const sunD = hypot(sunDx, sunDy) || 1;
    const tailNx = -sunDx / sunD, tailNy = -sunDy / sunD;

    const tailScreenPx = 280;
    const tailWorldLen = tailScreenPx / cam.zoom;

    ctx.save();
    const STEPS = 48;    for (let i = 1; i <= STEPS; i++) {
    ...
    for (const rock of a.rocks) {
      ...
      ctx.closePath();
      ctx.fillStyle = a.pal.fill; ctx.strokeStyle = a.pal.outline;
      ctx.lineWidth = 0.6 / cam.zoom; ctx.fill(); ctx.stroke();
      ctx.beginPath();
...
function initStars() {  stars = Array.from({ length: 180 }, () => ({
...
function addFlash(x, y, r, gc) {
  flashes.push({ x, y, r: r * 0.05, maxR: r * 3, gc, life: 0.55, speed: 0.14, kind: "ring" });
  flashes.push({ x, y, r: r * 0.1, maxR: r * 2, gc, life: 0.45, speed: 0.12, kind: "fill" });
  flashes.push({ x, y, r: 0, maxR: r * 0.8, gc, life: 0.60, speed: 0.10, kind: "core" });
}

function addNova(x, y, r, gc) {
  flashes.push({ x, y, r: r * 0.9, maxR: r * 1.6, gc, life: 0.9, speed: 0.18, kind: "white" });
  flashes.push({ x, y, r: r * 0.04, maxR: r * 3.5, gc, life: 0.70, speed: 0.11, kind: "ring" });
  flashes.push({ x, y, r: r * 0.08, maxR: r * 2.2, gc, life: 0.55, speed: 0.10, kind: "fill" });
  flashes.push({ x, y, r: r * 0.15, maxR: r * 1.3, gc, life: 0.65, speed: 0.09, kind: "fill" });
  flashes.push({ x, y, r: 0, maxR: r * 0.9, gc, life: 0.75, speed: 0.08, kind: "core" });
}
...
function drawFlashes() {
  for (let i = flashes.length - 1; i >= 0; i--) {
    ...
    if (f.kind === "white") {
      ...
      ctx.lineWidth = Math.max(0.3, r * 0.04 / cam.zoom); ctx.stroke();    } else if (f.kind === "ring") {
      ctx.beginPath(); ctx.arc(f.x, f.y, r, 0, PI2);
      ctx.strokeStyle = `rgba(${f.gc},${a * 0.22})`;
      ctx.lineWidth = Math.max(0.3, (f.maxR * 0.025) / cam.zoom) * a; ctx.stroke();
...
const screenToWorld = (sx, sy) => ({ x: cam.x + (sx - W / 2) / cam.zoom, y: cam.y + (sy - H / 2) / cam.zoom });const applyCam = () => { ctx.translate(W / 2, H / 2); ctx.scale(cam.zoom, cam.zoom); ctx.translate(-cam.x, -cam.y); };
const tickCam = () => { cam.zoom += (cam.targetZoom - cam.zoom) * 0.1; };
...
window.addEventListener("mousedown", e => { if (e.button === 1 || e.button === 2) { panning = true; panStart = { x: e.clientX, y: e.clientY }; camStart = { x: cam.x, y: cam.y }; e.preventDefault(); } });
window.addEventListener("mousemove", e => { if (panning) { cam.x = camStart.x - (e.clientX - panStart.x) / cam.zoom; cam.y = camStart.y - (e.clientY - panStart.y) / cam.zoom; } });
...
const spTrackPos = clientY => { const rect = spTrack.getBoundingClientRect(); setSpeed(clamp(1 - (clientY - rect.top) / rect.height, 0, 1) * SPEED_MAX); };
let spDrag = false;
document.getElementById("sp-fast").addEventListener("pointerdown", e => { e.preventDefault(); setSpeed(physSpeed + 0.5); });
document.getElementById("sp-slow").addEventListener("pointerdown", e => { e.preventDefault(); setSpeed(physSpeed - 0.5); });
...
window.addEventListener("mousemove", e => { tx = e.clientX; ty = e.clientY; });window.addEventListener("mousedown", e => { if (e.button !== 0 || uiEl.contains(e.target) || panning) return; holding = true; holdT = performance.now(); cursorEl.classList.add("holding"); });
...
document.getElementById("clear-btn").addEventListener("click", () => { bodies = []; loose = []; flashes = []; updateCount(); });
...
canvas.addEventListener("touchstart", e => {
  if (e.touches.length === 2) {
    ...
    pinchZoom0 = cam.targetZoom; pinchMidX = (a.clientX + b.clientX) / 2; pinchMidY = (a.clientY + b.clientY) / 2;
    e.preventDefault();
  } else if (e.touches.length === 1 && !holding) {
    const t = e.touches[0]; tx = t.clientX; ty = t.clientY;
    holding = true; holdT = performance.now(); cursorEl.classList.add("holding"); e.preventDefault();
  }
}, { passive: false });
...
function makeSpring(a, b, restLen, stiff, breakAt) {
  return { a, b, restLen, stiff: stiff || Config.SPRING_K, breakAt: breakAt || (restLen * Config.BREAK_MULT), broken: false };}
function makeBody(cx, cy, radius, pal) {
  ...
  for (const p of particles) {
    const d = hypot(p.x - cx, p.y - cy) / radius;
...
function integrateParticle(p, dt) {  if (p.dead) return;
...
// Optimized Broad-Phase Collision
      ...
      for (const cell of grid.values()) {
        ...
        for (const ea of cell) {
          ...
          for (const eb of cell) {
            ...
            const ov = Config.COLLISION_R - d, ma = pa.mass, mb = pb.mass, mt = ma + mb;
...
function spawnRing(body) {
  ...
  const ringW = body.radius * 0.6;  addFlash(rx, ry, body.radius * 4, body.pal.gc);
  ...
  for (let i = 0; i < RING_PARTICLES; i++) {
    ...
    const gmLocal = Config.GRAV_CONST * SUN.mass * (body.gravMult || sunGravMult);
    const vLocal = Math.sqrt(gmLocal / Math.max(r, 1));
    const scatter = rndR(0.96, 1.04);
    loose.push({ x: px, y: py, vx: -Math.sin(angle) * vLocal * scatter, vy: Math.cos(angle) * vLocal * scatter, mass: rndR(0.4, 1.2), pal: body.pal, heat: rndR(0.3, 0.8), life: rndR(0.7, 1.0), decay: rndR(0.0005, 0.002), isRing: true });
  }
}
...
function splitDeadParticles(body) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (!vis[i]) { loose.push({ x: p.x, y: p.y, vx: p.vx, vy: p.vy, mass: p.mass, pal: p.pal, heat: p.heat, life: 1, decay: rndR(0.004, 0.008) }); p.dead = true; }
    else alive++;
  }
  if (alive < Math.max(3, n * 0.08)) {
    for (const p of ps) if (!p.dead) loose.push({ x: p.x, y: p.y, vx: p.vx, vy: p.vy, mass: p.mass, pal: p.pal, heat: 1, life: 1, decay: rndR(0.005, 0.01) });
...
function looseVsPlanets() {
  ...
  for (let li = loose.length - 1; li >= 0; li -= step) {
    ...
    for (const body of bodies) {
      ...
      const thresh = body.radius + Config.LOOSE_HIT_R * 2;
      if (bd2 > thresh * thresh) continue;

      let nearP = null, nearD2 = Infinity;
      for (const bp of body.particles) { if (bp.dead) continue; const dx = bp.x - lp.x, dy = bp.y - lp.y; const d2 = dx * dx + dy * dy; if (d2 < nearD2) { nearD2 = d2; nearP = bp; } }      const nearD = Math.sqrt(nearD2);
      if (!nearP || nearD > Config.LOOSE_HIT_R) continue;
      ...
      if (Math.abs(vn) < 0.8) { nearP.vx += lp.vx * lp.mass / nearP.mass * 0.3; nearP.vy += lp.vy * lp.mass / nearP.mass * 0.3; nearP.heat = Math.min(1, nearP.heat + 0.25); lp.life = 0; }
        ...
        lp.x -= nx * (Config.LOOSE_HIT_R - nearD) * 0.9; lp.y -= ny * (Config.LOOSE_HIT_R - nearD) * 0.9;
        ...
        if (Math.abs(vn) > 2) {
          for (let k = 0; k < 3; k++) {
            ...
            loose.push({ x: lp.x, y: lp.y, vx: Math.cos(a) * s, vy: Math.sin(a) * s, mass: lp.mass * 0.15, pal: lp.pal, heat: 0.8, life: 0.6, decay: rndR(0.02, 0.04) });
          }
        }
      }
      break;
...
function tickLoose(dt) {
  if (loose.length > 400) loose = loose.slice(loose.length - 400);
  loose = loose.filter(p => p.life > 0);
  const gmScale = sunGravMult * 0.04;
  for (const p of loose) {
    const sdx = SUN.x - p.x, sdy = SUN.y - p.y;
    const sd2 = sdx * sdx + sdy * sdy, sd = Math.sqrt(sd2) + 0.1;
    if (sd < SUN.burnRadius) { p.life = 0; continue; }
    const sf = Config.GRAV_CONST * SUN.mass * gmScale / (sd2 + 500);
    p.vx += (sdx / sd) * sf * dt; p.vy += (sdy / sd) * sf * dt;
    for (const b of bodies) {
      const dx = b.cx - p.x, dy = b.cy - p.y, d2 = dx * dx + dy * dy, d = Math.sqrt(d2) + 0.1;
      const f = Config.GRAV_CONST * b.mass * (p.isRing ? 0.01 : 0.12) / (d2 + 150);
...
function tickBodies(scaledDt) {
  ...
  for (let sub = 0; sub < Config.SUBSTEPS; sub++) {
    for (let bi = 0; bi < bodies.length; bi++) { const body = bodies[bi], na = nAlives[bi]; for (const p of body.particles) if (!p.dead) applyGravity(p, na); }
    ...
    const burnSq = SUN.burnRadius * SUN.burnRadius;
    for (const body of bodies) for (const p of body.particles) { if (p.dead) continue; const dx = SUN.x - p.x, dy = SUN.y - p.y, sd2 = dx * dx + dy * dy; if (sd2 < burnSq) { p.dead = true; p.heat = 1; } else if (sd2 < burnSq * 6.25) p.heat = Math.min(1, p.heat + 0.04); }
...
// ============================================================================
// DRAWING: PLANETS & LOOSE
// ============================================================================
function drawLoose() {
  const hot = [], warm = [], cool = new Map(), ring = new Map();
  for (const p of loose) {
    const life = Math.min(p.life, 1), r = Math.max(0.01, Config.PARTICLE_R * (p.isRing ? 1.4 : 1) * life), a = life;
  ...
  const drawBatch = (style, arr) => { if (!arr.length) return; ctx.fillStyle = style; for (const [x, y, r, a] of arr) { ctx.globalAlpha = a; ctx.beginPath(); ctx.arc(x, y, r, 0, PI2); ctx.fill(); } };
  for (const [gc, pts] of ring) { ctx.shadowBlur = 3; ctx.shadowColor = `rgba(${gc},.6)`; drawBatch(`rgba(${gc},1)`, pts); ctx.shadowBlur = 0; }
...
function convexHull(pts) {
  ...
  let lo = pts[0]; for (const p of pts) if (p.y > lo.y || (p.y === lo.y && p.x < lo.x)) lo = p;
  ...
  while (true) {
    ...
    if (next === lo) break; hull.push(next); cur = next;
...
function drawBody(body) {
  ...
  if (hull.length > 2) {
    ctx.save(); ctx.beginPath(); ctx.moveTo(hull[0].x, hull[0].y);
    for (let i = 1; i < hull.length; i++) ctx.lineTo(hull[i].x, hull[i].y);
    ctx.closePath();
    const gr = ctx.createRadialGradient(body.cx, body.cy, 0, body.cx, body.cy, body.radius);
    gr.addColorStop(0, pal.hi + "dd"); gr.addColorStop(0.35, pal.mid + "cc"); gr.addColorStop(0.75, pal.lo + "bb"); gr.addColorStop(1, pal.lo + "44");
    ctx.fillStyle = gr; ctx.fill(); ctx.strokeStyle = `rgba(${pal.gc},.35)`; ctx.lineWidth = 1.5 / cam.zoom; ctx.stroke(); ctx.restore();
  }
  ctx.save(); ctx.globalAlpha = 0.08; ctx.strokeStyle = `rgba(${pal.gc},.9)`; ctx.lineWidth = 0.8 / cam.zoom; ctx.beginPath();
  for (const sp of ss) { if (sp.broken) continue; const pa = ps[sp.a], pb = ps[sp.b]; if (pa.dead || pb.dead) continue; if (hypot(pb.x - pa.x, pb.y - pa.y) / sp.restLen < 1.1) continue; ctx.moveTo(pa.x, pa.y); ctx.lineTo(pb.x, pb.y); }
  ctx.stroke(); ctx.restore();

  ctx.fillStyle = `rgba(${pal.gc},.75)`; ctx.beginPath();
  for (const p of alive) { if (p.heat > 0.05) continue; const r = p.isCore ? Config.PARTICLE_R * 1.3 : Config.PARTICLE_R; ctx.moveTo(p.x + r, p.y); ctx.arc(p.x, p.y, r, 0, PI2); p.heat = Math.max(0, p.heat - 0.012); }
  ctx.fill();
  for (const p of alive) { if (p.heat <= 0.05) continue; const r = p.isCore ? Config.PARTICLE_R * 1.3 : Config.PARTICLE_R; ctx.fillStyle = `rgba(255,${Math.floor(lerp(60, 220, p.heat))},30,${p.heat * 0.9})`; ctx.beginPath(); ctx.arc(p.x, p.y, r, 0, PI2); ctx.fill(); p.heat = Math.max(0, p.heat - 0.012); }
...
function drawSun(t) {
  ...
  SUN.coronaTime += 0.008; const ct = SUN.coronaTime;
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const rx = x + Math.cos(angle + Math.PI / 2) * radius * 0.12, ry = y + Math.sin(angle + Math.PI / 2) * radius * 0.12;    const fg = ctx.createLinearGradient(x, y, tipX, tipY);
    fg.addColorStop(0, `rgba(255,230,100,${0.25 * flicker})`); fg.addColorStop(0.5, `rgba(255,140,30,${0.15 * flicker})`); fg.addColorStop(1, "rgba(255,60,0,0)");
    ctx.beginPath(); ctx.moveTo(lx, ly); ctx.quadraticCurveTo(tipX + Math.cos(angle) * radius * 0.1, tipY + Math.sin(angle) * radius * 0.1, rx, ry); ctx.quadraticCurveTo(x + Math.cos(angle) * radius * 0.7, y + Math.sin(angle) * radius * 0.7, lx, ly);
    ctx.fillStyle = fg; ctx.fill();
  }
  ctx.restore();

  ctx.save(); ctx.beginPath(); ctx.arc(x, y, radius, 0, PI2); ctx.clip();
  ...
  for (const [a, d, r, fl] of [[0.8, 0.42, 0.09, 1.1], [2.3, 0.55, 0.06, 0.9], [4.1, 0.35, 0.07, 1.2], [5.5, 0.5, 0.05, 0.8]]) {
    ...
    sg.addColorStop(0, "rgba(30,10,0,.65)"); sg.addColorStop(0.6, "rgba(80,20,0,.35)"); sg.addColorStop(1, "rgba(0,0,0,0)");
  ...
  rim.addColorStop(0, "rgba(0,0,0,0)"); rim.addColorStop(0.4, "rgba(255,160,20,.12)"); rim.addColorStop(1, "rgba(255,40,0,0)");
  ...
  for (let i = 0; i < 6; i++) {
    ...
    ctx.beginPath(); ctx.moveTo(lx2, ly2); ctx.lineTo(ex, ey); ctx.lineTo(rx2, ry2); ctx.closePath(); ctx.fillStyle = sg; ctx.fill();
  }
  ctx.restore();
}

function drawCharge(t) {  if (!holding) return;
  const charge = Math.min((performance.now() - holdT) / 2000, 1);
...
function orbitalSpeed(dist, nP, grav) {
  ...
  return Math.sqrt(Config.GRAV_CONST * SUN.mass * g / Math.max(nP, 1) / Math.max(dist, 1));
}
function orbitalPeriod(dist, nP, grav) {
  const v = orbitalSpeed(dist, nP, grav); return v > 0 ? (2 * Math.PI * dist / v) : 99999;
}
function getSpawnVelocity(x, y, nP, grav) {
  const g = (grav != null) ? grav : sunGravMult; const dx = x - SUN.x, dy = y - SUN.y, dist = hypot(dx, dy) || 1;
  const v = orbitalSpeed(dist, nP, g); return { vx: -dy / dist * v, vy: dx / dist * v, dist };
}
function computePreviewDamping(periodSub) { return Math.pow(0.88, 1 / Math.max(periodSub, 1)); }
function predictOrbit(spawnX, spawnY, vx0, vy0, nP, steps, dtPerStep, recordEvery, grav) {
  ...
  const g = (grav != null) ? grav : sunGravMult, gm = Config.GRAV_CONST * SUN.mass * g / Math.max(nP, 1);
  const burnR2 = SUN.burnRadius * SUN.burnRadius; recordEvery = recordEvery || 1;
  const periodSub = orbitalPeriod(hypot(spawnX - SUN.x, spawnY - SUN.y) || 1, nP, g) / dtPerStep;
  const vDamp = computePreviewDamping(periodSub);
  for (let i = 0; i < steps; i++) {
    const sdx = SUN.x - px, sdy = SUN.y - py; const sd2 = sdx * sdx + sdy * sdy; if (sd2 < burnR2) break;
...
let _previewCache = null; const FIXED_NP = 100, PREVIEW_RECORD = 20, TARGET_ORBITS = 5;
function getPreviewPath(wx, wy) {
  if (_previewCache && Math.abs(_previewCache.wx - wx) < 2 && Math.abs(_previewCache.wy - wy) < 2 && _previewCache.mult === sunGravMult) return _previewCache.pts;
  const dist = hypot(wx - SUN.x, wy - SUN.y) || 1, dtPerStep = 1 / Config.SUBSTEPS;
  ...
  _previewCache = { wx, wy, pts, mult: sunGravMult }; return pts;}

function drawOrbitPreview(t) {
  if (!holding) return;
  const charge = Math.min((performance.now() - holdT) / 2000, 1), alpha = clamp(charge * 1.6, 0, 0.9);
  const w = screenToWorld(tx, ty), dx = w.x - SUN.x, dy = w.y - SUN.y, dist = hypot(dx, dy) || 1;
  const pts = getPreviewPath(w.x, w.y); if (pts.length < 4) return;
  const total = pts.length, lw = 1.8 / cam.zoom;
  ...
  for (let i = 0; i < total - SEG; i += SEG) {
    ...
    ctx.strokeStyle = `rgba(${r},${g},${b},${a})`; ctx.lineWidth = Math.max(0.3 / cam.zoom, w2); ctx.stroke();
  }
  ...
  for (let d = 0; d < DOT_COUNT; d++) {
    ...
    const dotR = Math.max(0.8 / cam.zoom, (3 - f * 1.5) / cam.zoom), da = alpha * (1 - f * 0.4) * 0.95;
    ctx.beginPath(); ctx.arc(pt.x, pt.y, dotR, 0, PI2); ctx.fillStyle = `rgba(${r2},${g2},${b2},${da})`; ctx.fill();
  }
  ctx.restore();

  ctx.save(); ctx.globalAlpha = alpha * 0.28; ctx.setLineDash([3 / cam.zoom, 4 / cam.zoom]);
  ctx.beginPath(); ctx.moveTo(SUN.x, SUN.y); ctx.lineTo(w.x, w.y); ctx.strokeStyle = "rgba(255,200,80,1)"; ctx.lineWidth = 0.6 / cam.zoom; ctx.stroke(); ctx.setLineDash([]); ctx.restore();

  const tx_ = -dy / dist, ty_ = dx / dist; const alen = Math.min(dist * 0.13, 240 / cam.zoom);
  ...
  ctx.beginPath(); ctx.moveTo(ax, ay); ctx.lineTo(ax - Math.cos(ha - 0.38) * hl, ay - Math.sin(ha - 0.38) * hl); ctx.lineTo(ax - Math.cos(ha + 0.38) * hl, ay - Math.sin(ha + 0.38) * hl); ctx.closePath(); ctx.fill(); ctx.restore();

  ctx.save(); ctx.globalAlpha = alpha; ctx.beginPath(); ctx.arc(w.x, w.y, 3.5 / cam.zoom, 0, PI2); ctx.fillStyle = "rgba(160,225,255,1)"; ctx.fill(); ctx.restore();

  const m = ctx.getTransform(); ctx.setTransform(1, 0, 0, 1, 0, 0); ctx.save();
  ...
  ctx.fillStyle = `rgba(180,215,255,${alpha * 0.65})`; ctx.fillText(`r${Math.round(dist)} ~${period_frames}f`, midSX + 8, midSY - 4); ctx.restore(); ctx.setTransform(m);
}

function spawnPlanet(x, y, size) {
  if (bodies.length >= 8) return;  const radius = clamp(size * 8, 16, 110), pal = PALS[Math.floor(rnd() * PALS.length)];
  const body = makeBody(x, y, radius, pal), nP = body.particles.length;
  body.gravMult = sunGravMult;
  const { vx, vy } = getSpawnVelocity(x, y, nP, sunGravMult);
  for (const p of body.particles) { p.vx = vx; p.vy = vy; }
  bodies.push(body); addFlash(x, y, radius * 2, pal.gc); updateCount();
}
function updateCount() { let n = 0; for (const b of bodies) if (!b.dead) n++; pcountEl.textContent = n === 0 ? "—" : `${n} WORLD${n !== 1 ? "S" : ""}`; }

// ============================================================================
// TRAIL BUFFERS & MAIN LOOP
// ============================================================================
const TRAIL_STEPS = 5; let trailBufs = [], trailHead = 0;
function initTrailBuffers() { trailBufs = Array.from({ length: TRAIL_STEPS }, () => { const c = document.createElement("canvas"); c.width = W; c.height = H; return { canvas: c, ctx: c.getContext("2d"), camX: 0, camY: 0, camZoom: 1, used: false }; }); }
function resizeTrailBuffers() { for (const b of trailBufs) { b.canvas.width = W; b.canvas.height = H; b.used = false; } }

function renderPlanetsToBuffer() {
  const buf = trailBufs[trailHead], ox = buf.ctx;
  ox.clearRect(0, 0, W, H); ox.save(); ox.translate(W / 2, H / 2); ox.scale(cam.zoom, cam.zoom); ox.translate(-cam.x, -cam.y);
  for (const b of bodies) {
    const { particles: ps, pal } = b; const alive = []; for (const p of ps) if (!p.dead) alive.push(p);
    if (alive.length < 3) continue; const hull = convexHull(alive); if (hull.length < 3) continue;
    ox.save(); ox.beginPath(); ox.moveTo(hull[0].x, hull[0].y); for (let i = 1; i < hull.length; i++) ox.lineTo(hull[i].x, hull[i].y); ox.closePath();
    const gr = ox.createRadialGradient(b.cx, b.cy, 0, b.cx, b.cy, b.radius);
    gr.addColorStop(0, pal.hi + "ff"); gr.addColorStop(0.35, pal.mid + "ee"); gr.addColorStop(0.75, pal.lo + "cc"); gr.addColorStop(1, pal.lo + "44");
    ox.fillStyle = gr; ox.fill(); ox.strokeStyle = `rgba(${pal.gc},.4)`; ox.lineWidth = 1.5 / cam.zoom; ox.stroke(); ox.restore();

    ox.save(); ox.globalAlpha = 0.07; ox.strokeStyle = `rgba(${pal.gc},.9)`; ox.lineWidth = 0.8 / cam.zoom; ox.beginPath();
    for (const sp of b.springs) { if (sp.broken) continue; const pa = ps[sp.a], pb = ps[sp.b]; if (pa.dead || pb.dead) continue; if (hypot(pb.x - pa.x, pb.y - pa.y) / sp.restLen < 1.1) continue; ox.moveTo(pa.x, pa.y); ox.lineTo(pb.x, pb.y); }
    ox.stroke(); ox.restore();

    ox.fillStyle = `rgba(${pal.gc},.75)`; ox.beginPath(); for (const p of alive) { if (p.heat > 0.05) continue; const r = p.isCore ? Config.PARTICLE_R * 1.3 : Config.PARTICLE_R; ox.moveTo(p.x + r, p.y); ox.arc(p.x, p.y, r, 0, PI2); } ox.fill();
    for (const p of alive) { if (p.heat <= 0.05) continue; const r = p.isCore ? Config.PARTICLE_R * 1.3 : Config.PARTICLE_R; ox.fillStyle = `rgba(255,${Math.floor(lerp(60, 220, p.heat))},30,${p.heat * 0.9})`; ox.beginPath(); ox.arc(p.x, p.y, r, 0, PI2); ox.fill(); }

    const atm = ox.createRadialGradient(b.cx, b.cy, b.radius * 0.7, b.cx, b.cy, b.radius * 1.8);
    atm.addColorStop(0, `rgba(${pal.gc},.07)`); atm.addColorStop(1, `rgba(${pal.gc},0)`); ox.fillStyle = atm; ox.beginPath(); ox.arc(b.cx, b.cy, b.radius * 1.8, 0, PI2); ox.fill();
  }
  ox.restore(); buf.camX = cam.x; buf.camY = cam.y; buf.camZoom = cam.zoom; buf.used = true;
}
...
function drawTrail() {
  for (let age = TRAIL_STEPS - 1; age >= 0; age--) {
    ...
    const buf = trailBufs[idx]; if (!buf.used) continue;
    const alpha = TRAIL_ALPHAS[age]; if (alpha < 0.005) continue;
    const scaleRatio = cam.zoom / buf.camZoom, offX = (buf.camX - cam.x) * cam.zoom, offY = (buf.camY - cam.y) * cam.zoom;
    ctx.save(); ctx.globalAlpha = alpha;
    ctx.globalCompositeOperation = age === 0 ? "source-over" : "lighter";
    ctx.translate(W / 2 + offX, H / 2 + offY); ctx.scale(scaleRatio, scaleRatio); ctx.translate(-W / 2, -H / 2);    ctx.drawImage(buf.canvas, 0, 0); ctx.restore();
  }
  ctx.globalCompositeOperation = "source-over";
}

let fpsSamples = [], fpsDisplay = 0;
function updateFPS(rawDt) { if (rawDt > 0) { fpsSamples.push(1 / rawDt); if (fpsSamples.length > 30) fpsSamples.shift(); } if (fpsSamples.length > 0) fpsDisplay = Math.round(fpsSamples.reduce((a, b) => a + b) / fpsSamples.length); }
function drawFPS() { ctx.save(); ctx.font = '10px "Space Mono",monospace'; ctx.fillStyle = "rgba(100,255,160,0.55)"; ctx.letterSpacing = ".05em"; ctx.fillText(`${fpsDisplay} FPS`, 16, 20); ctx.restore(); }

initStars(); initTrailBuffers();
window.addEventListener("resize", () => { W = canvas.width = window.innerWidth; H = canvas.height = window.innerHeight; initStars(); resizeTrailBuffers(); });
ctx.fillStyle = "#04040c"; ctx.fillRect(0, 0, W, H);

let lastT = 0;
function loop(t) {
  requestAnimationFrame(loop);
  const rawDt = Math.min((t - lastT) / 1000, 0.05); lastT = t;
  updateFPS(rawDt); tickCam(); updateSpeedBar(); updateZoomBar();
  ctx.fillStyle = "rgba(4,4,12,.28)"; ctx.fillRect(0, 0, W, H);
  drawNebula(t); drawStars(t);
  ctx.save(); applyCam();
  if (!paused && physSpeed > 0 && rawDt > 0) {
    const MAX_SAFE_SD = rawDt * 3.0 * Config.PHYS_SCALE, totalSd = rawDt * physSpeed * Config.PHYS_SCALE;
    const numTicks = Math.ceil(totalSd / MAX_SAFE_SD), sdPerTick = totalSd / numTicks;
    for (let tick = 0; tick < numTicks; tick++) { tickBodies(sdPerTick); tickLoose(sdPerTick); }
    tickAsteroids(rawDt * physSpeed * Config.PHYS_SCALE);
  }
  drawFlashes(); drawSun(t); drawLoose(); drawAsteroids(); drawOrbitPreview(t);
  ctx.restore();
  renderPlanetsToBuffer(); drawTrail(); trailHead = (trailHead + 1) % TRAIL_STEPS;
  drawCharge(t); drawFPS();
  ctx.fillStyle = "rgba(180,210,255,.15)"; ctx.font = '8px "Space Mono",monospace'; ctx.letterSpacing = ".18em";
  ctx.fillText(`${(cam.zoom * 100).toFixed(0)}% SCROLL·ZOOM RIGHT-DRAG·PAN F·FIT SPACE·PAUSE`, 16, H - 16);
  cursorEl.style.left = tx + "px"; cursorEl.style.top = ty + "px"; updateCount();
}
requestAnimationFrame(loop);